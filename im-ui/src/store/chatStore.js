	...
	mutations: {
		...
		openChat(state, chatInfo) {
			...
			if (chat == null) {
				...
				state.chats.unshift(chat);console.log(state.chats);
			}
			// 选中会话保持不变
			if(activeChat){
		...
		insertMessage(state, msgInfo) {
			...
			let targetId = msgInfo.groupId ? msgInfo.groupId : msgInfo.selfSend ? msgInfo.receiveId : msgInfo.sendId;
			let chat = null;
			for (let idx in state.chats) {
		...
		updateChatFromFriend(state, friend) {
			for (let i in state.chats) {
				...
				if (chat.type=='PRIVATE' && chat.targetId == friend.friendId) {
		...
		updateChatFromGroup(state, group) {
			for (let i in state.chats) {
				...
				if (chat.type=='GROUP' && chat.targetId == group.id) {
					chat.headImage = group.imageThumb;
