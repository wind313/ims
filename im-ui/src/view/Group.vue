						...
						<div>
							<file-upload class="avatar-uploader" :action="imageAction" :disabled="!isOwner" :showLoading="true"
							 :maxSize="maxSize" @success="handleUploadSuccess" :fileTypes="['image/jpeg', 'image/png', 'image/jpg','image/webp']">
								<img v-if="activeGroup.image" :src="activeGroup.image" class="avatar">
					...
					<el-scrollbar style="height:400px;">
						<div class="r-group-member-list">
							<div v-for="(member) in groupMembers" :key="member.id">
								<group-member v-show="!member.quit" class="r-group-member" :member="member" :showDel="isOwner&&member.memberId!=activeGroup.ownerId"
	...
	export default {
		...
		methods: {
			...
			handleUploadSuccess(res) {
				this.activeGroup.image = res.data.url;
				this.activeGroup.imageThumb = res.data.thumbUrl;
			},
			handleSaveGroup() {
			...
			handleSendMessage() {
				let chat = {
					...
					headImage: this.activeGroup.imageThumb,
				};
		...
		computed: {
			...
			ownerName() {
				...
				let member = this.groupMembers.find((m) => m.memberId == this.activeGroup.ownerId);
